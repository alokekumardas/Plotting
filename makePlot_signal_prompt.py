...
if tight:      #SR8 
	...
	if selYear  =='2016': ZJetSF = 1.22; MisIDEleSF,ZGammaSF,WGammaSF = getFullYearMisIDEleSF(selYear,isSelection); # use misIDEl for each year but same V sf for all year.
	elif selYear=='2017': ZJetSF = 1.22; MisIDEleSF,ZGammaSF,WGammaSF = getFullYearMisIDEleSF(selYear,isSelection);
	else :                ZJetSF = 1.22; MisIDEleSF,ZGammaSF,WGammaSF = getFullYearMisIDEleSF(selYear,isSelection);
	if channel=='ele':
...
if M3Plot:
	...
	mydistributionName = histNameData[7:-3]+"pho0"
else:
...
if mydistributionName == "ChIso":
	...
	rebinLeft =0.5 # 10 #20
	rebinRight = 3
	rebinLeftRightRight =4 # 10 #20
	#binning  = numpy.arange(50,500.1,rebin)

	binningLeft   = list(numpy.arange(0,2.1,rebinLeft)) # 40, 20 start
	binningRight  = list(numpy.arange(2.5,10.1,rebinRight))
	binningRightRight  = list(numpy.arange(12,20.1,rebinLeftRightRight))
	binning = numpy.array(binningLeft + binningRight+ binningRightRight)
...
if postfitPlots:
	...
	rebinnedHist["nonPromptTTGamma"].Scale(TTGammaSF)
