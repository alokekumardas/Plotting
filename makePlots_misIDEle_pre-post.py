...
parser.add_option("--plotWithSyst", dest="plotWithSyst", default=False,action="store_true",
					help="plot with Systematics" )
...
plotWithSyst = options.plotWithSyst
...
if plotWithSyst:
	if finalState=='Ele':
		systematicsList = ["PU","BTagSF_l","PhoEff", "BTagSF_b"]

	else:
		systematicsList = ["PU","MuEff","BTagSF_l","PhoEff", "BTagSF_b"]
...
if tight:      #SR8 
	...
	if systematics in allsystematics:
		...
		plotDirectory = "misIDEle_PrePost_tightplots_%s_%s/"%(channel,selYear)
...
if looseCRge2ge0:  #AR
	...
	if systematics in allsystematics:
		...
		plotDirectory = "misIDEle_PrePost_looseCRge2ge0plots_%s_%s/"%(channel,selYear)
	else:
...
if looseCRge2e0:  #CR1+CR2+CR3
	...
	if systematics in allsystematics:
		...
		plotDirectory = "misIDEle_PrePost_looseCRge2e0plots_%_%s/"%(channel,selYear)		
	else:
		fileDir  = "histograms_%s/%s/hists_looseCRge2e0/"%(selYear, channel)
		plotDirectory = "misIDEle_PrePost_looseCRge2e0plots_%s_%s/"%(channel,selYear)
...
if looseCRe2e0:  #CR1
	...
	if systematics in allsystematics:
		...
		plotDirectory = "misIDEle_PrePost_looseCRe2e0plots_%s_%s/"%(channel,selYear)
	else:
...
if looseCRe3e0:  #CR2
	...
	if systematics in allsystematics:
		...
		plotDirectory = "misIDEle_PrePost_looseCRe3e0plots_%s_%s/"%(channel,selYear)
	else:
...
if looseCRge4e0:  #CR3
	...
	if systematics in allsystematics:
		...
		plotDirectory = "misIDEle_PrePost_looseCRge4e0plots_%s_%s/"%(channel,selYear)
	else:
...
if looseCRe2e1:  #CR4
	...
	if systematics in allsystematics:
		...
		plotDirectory = "misIDEle_PrePost_looseCRe2e1plots_%s_%s/"%(channel,selYear)
	else:
...
if looseCRe3e1:  #CR5
	...
	if systematics in allsystematics:
		...
		plotDirectory = "misIDEle_PrePost_looseCRe3e1plots_%s_%s/"%(channel,selYear)
	else:	
...
if looseCRe2e2:  #CR6
	...
	if systematics in allsystematics:
		...
		plotDirectory = "misIDEle_PrePost_looseCRe2e2plots_%s_%s/"%(channel,selYear)
	else:	
...
if looseCRe3ge2:  #CR7
	...
	if systematics in allsystematics:
		...
		plotDirectory = "misIDEle_PrePost_looseCRe3ge2plots_%s_%s/"%(channel,selYear)
	else:
...
for sample in sampleList:
	...
	#	_filesys_up  [sample][syst]=TFile.Open(eosFolder+"histograms_%s/%s/hists_%s_up_%s/%s.root"%(selYear,channel,syst,isSelectionDir,sample),"read")
	#	_filesys_down[sample][syst]=TFile.Open(eosFolder+"histograms_%s/%s/hists_%s_down_%s/%s.root"%(selYear,channel,syst,isSelectionDir,sample),"read")
		#print "histograms_%s/%s/hists_%s_up_%s/%s.root"%(selYear,channel,syst,isSelectionDir,sample)
...
if postfitPlots:
	...
	x = rebinnedData.Chi2Test(rebinnedMC,"UU CHI2/NDF") 
